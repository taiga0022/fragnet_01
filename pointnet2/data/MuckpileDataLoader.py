...
class MuckpileData(data.Dataset):
    def __init__(self, num_points, transforms=None, train=True, download=True):
        ...
        self._cache = os.path.join(BASE_DIR, "MuckpileData_thesis_cache") #ここも適宜変える

        #はじめてzipを開くとき、LMDBをつくる。
        if not osp.exists(self._cache):
            self.folder = "MuckpileDatas_thesis" #ここも適宜変える
            self.data_dir = os.path.join(BASE_DIR, self.folder)
            
            if download and not os.path.exists(self.data_dir):
                zipfile = os.path.join(BASE_DIR, "MuckpileDatas_thesis.zip") #アップしたzipファイル
            ...
            for split in ["train", "test"]:
                if split == "train":
                    shape_ids = [
                        line.rstrip()
                        for line in open(
                            os.path.join(self.data_dir, "muckpile_train.txt")
                        )
                    ]
                else:
                    shape_ids = [
                        line.rstrip()
                        for line in open(
                            os.path.join(self.data_dir, "muckpile_test.txt")
                        )
                    ]
                ...
                # list of xyz_file_path, label_file_path) tuple
                self.datapath = [
                    (
                        os.path.join(self.data_dir, "xyz", shape_ids[i])
                        + ".txt",
                        os.path.join(self.data_dir, "label", shape_ids[i])
                        + ".csv",
                    )
                ...
                with lmdb.open(
                    ...
                    for i in tqdm.trange(len(self.datapath)):
                        fn = self.datapath[i]
                        #ここはnp.loadtxtのままで良いか要検討
                        point_set = np.loadtxt(fn[0], delimiter=" ").astype(np.float32)
                        label = np.loadtxt(fn[1], delimiter=",", skiprows=1).astype(np.float32)
                        '''
                        ...
                        txn.put(
                            str(i).encode(),
                            msgpack_numpy.packb(
                                dict(pc=point_set, lbl=label), use_bin_type=True
                            ),
                        )
    ...
    def __getitem__(self, idx):
        ...
        point_set = pc_normalize(point_set[:, 0:3]) #もうｘｙｚにしちゃう。
...
if __name__ == "__main__":
    ...
    dset = MuckpileData(50000, train=True, transforms=transforms)
    print(dset[0][0])
    print(dset[0][1])
    print(len(dset))
