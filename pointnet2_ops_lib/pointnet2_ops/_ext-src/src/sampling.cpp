...
void furthest_point_sampling_kernel_wrapper(int b, int n, int m, float r,
...
at::Tensor furthest_point_sampling(at::Tensor points, const int nsamples, const float r) {
  CHECK_CONTIGUOUS(points);
  CHECK_IS_FLOAT(points);
  ...
  if (points.is_cuda()) {
    furthest_point_sampling_kernel_wrapper(
        points.size(0), points.size(1), nsamples, r, points.data_ptr<float>(),
